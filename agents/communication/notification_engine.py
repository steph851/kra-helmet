...
class NotificationEngine(BaseAgent):
    ...
    def _wa_en(self, name: str, obligations: list, emoji: str, prefix: str) -> str:
        ...
        lines.append("")
        lines.append("\U0001f449 *Two ways to file:*")
        lines.append("1\ufe0f\u20e3 iTax: https://itax.kra.go.ke")
        lines.append(f"2\ufe0f\u20e3 WhatsApp: Send 'Hi' to +254711099999 (KRA Shuru)")
        return "\n".join(lines)
    ...
    def _wa_sw(self, name: str, obligations: list, emoji: str, prefix: str) -> str:
        ...
        lines.append("")
        lines.append("\U0001f449 *Njia mbili za kuwasilisha:*")
        lines.append("1\ufe0f\u20e3 iTax: https://itax.kra.go.ke")
        lines.append("2\ufe0f\u20e3 WhatsApp: Tuma 'Hi' kwa +254711099999 (KRA Shuru)")
        return "\n".join(lines)
    ...
    def format_sms(
        ...
        if lang == "sw":
            if days is not None and days < 0:
                msg = f"{name}: {tax} IMECHELEWA siku {abs(days)}! Wasilisha iTax au WhatsApp 0711099999"
            elif days is not None and days == 0:
                msg = f"{name}: {tax} INAISHA LEO! Wasilisha iTax au WhatsApp 0711099999"
            else:
                msg = f"{name}: {tax} siku {days} ({dl}). iTax au WhatsApp 0711099999"
        else:
            if days is not None and days < 0:
                msg = f"{name}: {tax} OVERDUE {abs(days)}d! File via iTax or WhatsApp 0711099999"
            elif days is not None and days == 0:
                msg = f"{name}: {tax} DUE TODAY! File via iTax or WhatsApp 0711099999"
            else:
                msg = f"{name}: {tax} due {days}d ({dl}). iTax or WhatsApp 0711099999"
    ...
    def format_email(
        ...
        subject = f"[KRA Deadline Tracker] {prefix}: Tax obligations for {business}"
        ...
        text_lines += [
            "",
            "Two ways to file:",
            "  1. iTax: https://itax.kra.go.ke",
            "  2. KRA WhatsApp (Shuru): Send 'Hi' to +254 711 099 999 on WhatsApp",
            "     File returns, pay taxes, and get compliance certificates — all in 3 steps.",
            "",
            "---",
            "DISCLAIMER: This is an automated reminder generated by KRA Deadline Tracker.",
            ...
            "",
            "KRA Deadline Tracker — Protecting Kenyan SMEs from tax surprises.",
        ]
        body_text = "\n".join(text_lines)
...
<ol>
<li><a href="https://itax.kra.go.ke">File on iTax</a></li>
<li><a href="https://wa.me/254711099999">File via KRA WhatsApp (Shuru)</a> — Send 'Hi' to +254 711 099 999. File returns, pay taxes, get compliance certificates in 3 easy steps.</li>
</ol>
<hr>
